...
3. 读取 `UPGRADE-SOURCE-COMMIT-CHECKLIST-155.md` 的“当前位置”：当前已分析到第 `125 / 155`；第 `93`、`95 / 155` 已本地提交 `a31f12f8`，第 `96`、`100 / 155` 已本地提交 `fd5f3e1a`，第 `101 / 155` 已补同步测试工程并由用户验证通过，已本地提交；第 `107 / 155` 已补同步测试工程并由用户验证通过，已回写主仓；当前分支相对 `origin/ups` 尚未推送；第 `94 / 155` 因 Harmony selection/caret 坐标能力限制暂不同步；第 `28 / 155`、`75 / 155` 经 RNOH window API 复查判断可实现 `windowDidResize` 但需真机验证；第 `108 / 155` 经测试工程验证确认 Harmony decorator 只能增加拖拽视觉距离，不能增加稳定 scroll range，当前能力缺失不能完全做；第 `109`、`111`、`117`、`119`、`121`、`122 / 155` 依赖 108/109 稳定 content inset 或未暴露的 `KeyboardChatScrollView`，暂不同步；第 `113 / 155` 依赖上游新 KASV selection-on-focus 链路，当前暂不同步；第 `123`、`125 / 155` 的 KAV 自动偏移链有真实 Harmony 原生承接路径但需测试工程验证；第 `112`、`114-116`、`118`、`120 / 155` 不需同步；下一笔是第 `126 / 155` `6872736017`。
4. 查询上游真实 diff：
...
- 小批次可以一次分析连续 2-3 笔；若遇到需要 JS/原生验证的关键改动，停下来让用户测，不要越过验证点继续处理后续功能。

## 当前关键状态

- 当前待测试候选：第 `28 / 155`、`75 / 155` 可做 Harmony `windowDidResize` 事件链路试验验证；第 `123 / 155`、`125 / 155` 可做 `KeyboardAvoidingView automaticOffset` + native `viewPositionInWindow` 测试工程验证。第 `101 / 155`、`107 / 155` 已在测试工程验证通过并回写主仓；第 `108 / 155` 的 Harmony 原生 decorator 已在测试工程验证为当前能力缺失不能完全做，不进入主仓；第 `102-106`、`110`、`112`、`114-116`、`118`、`120 / 155` 已分析为当前不需同步；第 `124 / 155` 是可同步纯 JS/TS 改动，后续也需先进入测试工程或随 KAV 测试链验证；第 `109`、`111`、`113`、`117`、`119`、`121`、`122 / 155` 暂不同步。
- 最近本地提交：第 `93`、`95 / 155` 已提交为 `a31f12f8 同步 useAnimatedKeyboard 兼容接口`；第 `96`、`100 / 155` 已提交为 `fd5f3e1a 同步 KASV deps 修复和 Provider 类型导出`；第 `101 / 155` 已提交为 `同步 KAV 非 iOS 位移条件`；第 `107 / 155` 本轮回写主仓；当前分支仍未推送 `origin/ups`。
...
截至第 `125 / 155` 笔源码相关 commit 已按时间顺序完成证据审计，最后分析的是 `eb2dfbbe35`。第 `93`、`95 / 155` 笔已本地提交为 `a31f12f8`；第 `96`、`100 / 155` 笔已本地提交为 `fd5f3e1a`；第 `101 / 155` 已本地提交且已补同步到测试工程验证通过；第 `107 / 155` 已同步到测试工程验证通过并回写主仓；当前分支相对 `origin/ups` 仍未推送。第 `123 / 155`、`125 / 155` 是后续需要测试工程验证的 KAV 自动偏移链；下一笔是第 `126 / 155` 笔：`6872736017`。
...
| 本批待验证 | 第 `28 / 155`、`75 / 155` 可在测试工程桥接 Harmony `WINDOW_SIZE_CHANGE` -> `KeyboardController::windowDidResize` 并同步最终 `useWindowDimensions` 逻辑验证；第 `123 / 155`、`125 / 155` 可做 `KeyboardAvoidingView automaticOffset` + native `viewPositionInWindow` 测试工程验证。第 `101 / 155`、`107 / 155` 已在测试工程验证通过并回写主仓；第 `108 / 155` 已验证当前能力缺失不能完全做，不进入主仓；第 `109`、`111`、`117`、`119 / 155` 依赖第 108 稳定 bottom/top inset，暂不同步；第 `113 / 155` 依赖上游新 KASV selection-on-focus 链路，当前暂不同步。 |
...
从第 `126 / 155` 笔 `6872736017` 继续按 5 笔一批分析；第 `107 / 155` 的 `useCombinedRef` 已按流程先进入测试工程、验证通过后回写主仓；第 `123 / 155`、`124 / 155`、`125 / 155` 可作为一个 KAV automaticOffset 测试工程任务推进。第 `108 / 155` / `109 / 155` / `111 / 155` / `117 / 155` / `119 / 155` / `121 / 155` / `122 / 155` 当前不进入主仓，除非后续 RNOH ScrollView 暴露稳定 content inset / content container padding 能力。
