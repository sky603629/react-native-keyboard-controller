...
3. 读取 `UPGRADE-SOURCE-COMMIT-CHECKLIST-155.md` 的“当前位置”：当前已分析完第 `155 / 155`；第 `127 / 155` `KeyboardToolbar.Group` 已完成测试工程验证并回写主仓；第 `117 / 155` 基础 `KeyboardChatScrollView` 已回写上游 JS，但 Harmony spacer 验证结论已撤销，当前因原生 contentInset/稳定滚动范围缺失标记为不能完全做；测试工程保留原样 JS demo 用于观察真实差异。第 `28 / 155`、`75 / 155` 可做 `windowDidResize` 真机验证；第 `139 / 155`、`147 / 155` 是待测试候选；第 `124 / 155` 是可同步纯类型候选但不能夹带 `automaticOffset`；第 `146 / 155` 已试验并撤销库和 demo 修改，暂不同步；第 `108 / 155` 经测试工程验证为当前能力缺失不能完全做；第 `123`、`125`、`126`、`130 / 155` 经测试工程验证普通 RN View tag 无法稳定测窗口坐标，暂不同步；其他状态以总清单逐笔结论为准；后续没有未分析源码 commit。
4. 查询上游真实 diff：
...
- `KeyboardChatScrollView`：第 `117 / 155` 的公共 API、组件结构和内部 JS 已恢复为上游原样实现，不再保留 Harmony spacer、强制 `inverted=false`、内容高度扣减或 `useScrollViewOffset` 替代逻辑。当前 Harmony 的 `ScrollView` 没有承接动态 `contentInset.top/bottom` 和稳定扩展滚动范围，因此 `always`、`whenAtEnd`、`persistent`、`offset`、`inverted` 等依赖 inset 的语义不能与 iOS/Android 等价；这属于原生底座缺失，禁止再用 JS 兼容分支掩盖。`useEventHandlerRegistration` 已确认兼容测试工程的 Reanimated 4.2.1，不再视为 RNOH 缺失。后续 Chat 属性仍按上游 commit 顺序同步 JS，并分别记录 Harmony 原生能力结论。
...
截至第 `155 / 155` 笔源码相关 commit 已按时间顺序完成证据审计，最后分析的是 `38076a9df8`。第 `93`、`95 / 155`、第 `96`、`100 / 155`、第 `101 / 155`、第 `107 / 155` 已完成测试工程验证、回写主仓并推送到 `origin/ups`；第 `127 / 155` `KeyboardToolbar.Group` 已完成测试工程验证并回写主仓，尚未推送。第 `117 / 155` 基础 `KeyboardChatScrollView` 已回写上游 JS，但本轮已撤销 Harmony JS workaround 和相应“验证通过”结论，功能因原生 contentInset/稳定滚动范围缺失不能完全做。第 `28 / 155`、`75 / 155` 是 `windowDidResize` 可实现待验证候选；第 `124 / 155` 是纯 TS 类型可同步候选；第 `139 / 155` 是新增 `keyboardBorderRadius` 常量接口的待测试候选；第 `147 / 155` 是 Toolbar touch bounds 功能修复候选。后续没有未分析源码 commit。
...
- 已分析第 `146-150 / 155` 笔：第 146 `b3dc83edd5` 的 opening dismiss 已在 Harmony 真机试验后撤销；第 147 `15111a4213` Toolbar 固定 touch bounds 待测试；第 148 `c436fbb9e0` 只修 inverted + FlashList，Harmony JS 不禁用 `inverted`，但原生 top inset/负滚动范围缺失使该组合不能等价实现；第 149 `1048f5238a` 是未同步 viewPositionInWindow 的 Android 防崩溃；第 150 `ce8ccaa5f7` 的回调 JS 可计算目标 inset，但原生未实际应用，当前不能对外承诺等价状态。
- 已分析第 `151-155 / 155` 笔：第 151 `21a939f30d` 的 Reanimated StickyView 目标已由 Harmony 当前实现覆盖；第 152 `5dc56430d7` 的 `onEndVisible` 使用上游 `useEvent` + `useEventHandlerRegistration`，测试工程 Reanimated 4.2.1 具备 `registerForEvents/unregisterFromEvents`，此前“RNOH 缺少事件注册 helper”的结论已纠正；该回调可进入真机验证，不需要 Harmony JS fallback。第 153、154 分别是 iOS weak keyboard view 与 Catalyst swizzle guard；第 155 是 Android IME onStart 重入修复，均不适用于当前 Harmony 链路。

## 下一步

`1.16.5..1.21.8` 的 `155` 笔源码相关 commit 已全量审计完毕。第 `117 / 155` 基础 `KeyboardChatScrollView` 的 JS 已按上游原样回写主仓和测试工程，但 Harmony 当前因 RNOH 稳定 top/bottom `contentInset` 与合法滚动范围缺失，不能将先前 spacer 试验结果计为正式支持。下一步仍按时间顺序同步第 `121 / 155` helper 初始化修复；后续公开 JS API 保持上游一致，逐项只在原生侧补齐，无法补齐的明确记录为 No。`onEndVisible` 的 Reanimated event registration 已确认具备代码兼容条件，可进入真机验证。
