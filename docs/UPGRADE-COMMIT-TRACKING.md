...
3. 读取 `UPGRADE-SOURCE-COMMIT-CHECKLIST-155.md` 的“当前位置”：当前已分析完第 `155 / 155`；第 `117 / 155` 基础 `KeyboardChatScrollView` 和第 `127 / 155` `KeyboardToolbar.Group` 已完成测试工程验证并回写主仓，测试工程保留对应 demo；第 `28 / 155`、`75 / 155` 可做 `windowDidResize` 真机验证；第 `139 / 155`、`147 / 155` 是待测试候选；第 `124 / 155` 是可同步纯类型候选但不能夹带 `automaticOffset`；第 `146 / 155` 已试验并撤销库和 demo 修改，暂不同步；第 `108 / 155` 经测试工程验证为当前能力缺失不能完全做；第 `123`、`125`、`126`、`130 / 155` 经测试工程验证普通 RN View tag 无法稳定测窗口坐标，暂不同步；第 `109`、`111`、`113`、`119`、`121`、`122`、`128`、`131-136`、`138`、`142`、`145`、`148-150`、`152 / 155` 暂不同步；第 `102-106`、`110`、`112`、`114-116`、`118`、`120`、`129`、`137`、`140`、`141`、`143`、`144`、`151`、`153-155 / 155` 不需同步；后续没有未分析源码 commit。
4. 查询上游真实 diff：
...
- 当前待测试候选：第 `28 / 155`、`75 / 155` 可做 Harmony `windowDidResize` 事件链路试验验证；第 `139 / 155` 可补 `getConstants().keyboardBorderRadius=0` 和 `KEYBOARD_BORDER_RADIUS` 验证 Toolbar rounded 判断；第 `147 / 155` 可验证 `KeyboardToolbar` 外层 fixed bounds 后按钮点击不穿透。第 `101 / 155`、`107 / 155`、`117 / 155`、`127 / 155` 已在测试工程验证通过并回写主仓；第 `117 / 155` 当前只确认普通非 inverted、默认 `keyboardLiftBehavior="always"` 路线，Chat 后续属性继续按 commit 顺序验证；对应 Chat 和 Group demo 继续保留在测试工程。第 `108 / 155` 的 Harmony 原生 decorator 已在测试工程验证为当前能力缺失不能完全做，不进入主仓；第 `123`、`125`、`126`、`130 / 155` 已在测试工程验证为当前普通 RN View tag 测量能力缺失，不能通过改 JS wrapper 绕行，暂不同步；第 `146 / 155` 测试工程试验后已撤销库和 demo 修改，暂不同步：Harmony focus 后系统快速发 `keyboardWillHide`，未稳定进入 `KeyboardController.dismiss()` 原生链路，不能验证上游 opening dismiss 语义；第 `124 / 155` 是可同步纯 JS/TS 类型改动，但不能夹带 `automaticOffset` 能力承诺；第 `109`、`111`、`113`、`119`、`121`、`122`、`128`、`131-136`、`138`、`142`、`145`、`148-150`、`152 / 155` 暂不同步。
...
- `KeyboardChatScrollView`：第 `117 / 155` 基础公开组件已通过 Harmony spacer 降级落地并完成真机验证，支持普通非 inverted、默认 `keyboardLiftBehavior="always"` 路线。第 `108 / 155` 的原生 Clipping 无布局 inset 语义仍为 No，Harmony `inverted` 仍不支持；`extraContentPadding` / `blankSpace` / SharedValue `freeze` / `onContentInsetChange` / `onEndVisible` 等后续能力按原提交顺序逐项验证，不随基础组件一次性引入。
- `KeyboardAvoidingView automaticOffset`：第 `123 / 155` 新增 public prop，第 `125 / 155` 新增内部 native `viewPositionInWindow` 来替代不可靠的 RN `measureInWindow`。测试工程验证表明，当前 Harmony 原生侧用 `RNInstanceCAPI::findComponentInstanceByTag(tag)` 只能稳定承接部分可控 native component，无法稳定解析 KAV 上游 JS 传入的普通 RN View/Reanimated View tag，日志表现为 `Component instance was not found`。按“JS 尽量保持上游一致，只改原生”的原则，不允许改 KAV JS 渲染 wrapper 来绕过该限制；因此第 `123`、`125 / 155` 标记为 Harmony 当前能力缺失暂不同步。
- `KeyboardAwareScrollView` top border auto detect：第 `126 / 155`、`130 / 155` 是第 `125` 的直接下游，依赖 `KeyboardControllerNative.viewPositionInWindow(scrollViewTag)` 测普通 RN/Reanimated ScrollView tag；当前同样暂不同步。未来如果 RNOH 暴露稳定普通 tag 测量 API，需要把第 `126` 的 `scrollViewPageY` 与第 `130` 的 try/catch 一起同步。
...
截至第 `155 / 155` 笔源码相关 commit 已按时间顺序完成证据审计，最后分析的是 `38076a9df8`。第 `93`、`95 / 155`、第 `96`、`100 / 155`、第 `101 / 155`、第 `107 / 155` 已完成测试工程验证、回写主仓并推送到 `origin/ups`；第 `117 / 155` 基础 `KeyboardChatScrollView` 和第 `127 / 155` `KeyboardToolbar.Group` 已完成测试工程验证并回写主仓，尚未推送。第 `28 / 155`、`75 / 155` 是 `windowDidResize` 可实现待验证候选；第 `124 / 155` 是纯 TS 类型可同步候选；第 `139 / 155` 是新增 `keyboardBorderRadius` 常量接口的待测试候选；第 `147 / 155` 是 Toolbar touch bounds 功能修复候选。第 `136`、`138`、`142`、`145`、`146`、`148-150`、`152 / 155` 受 ChatScrollView/contentInset/KASV 新状态机、Harmony focus 后系统快速 hide 时序或普通 tag 测量链限制暂不同步；第 `137`、`140`、`141`、`143`、`144`、`151`、`153-155 / 155` 当前无 Harmony 同类落点或已由当前实现覆盖，不需同步；后续没有未分析源码 commit。
...
| 本批状态 | 第 `127 / 155` 已完成 `KeyboardToolbar.Group` marker component、traversal 边界和深度焦点扫描验证并回写主仓；测试工程独立 demo 保留。第 `28 / 155`、`75 / 155` 可验证 `windowDidResize`；第 `139 / 155`、`147 / 155` 仍是 Toolbar 待测试候选。第 `108 / 155` 已验证当前能力缺失不能完全做；第 `123`、`125`、`126`、`130 / 155` 因普通 RN View tag 测量限制暂不同步；第 `146 / 155` 已试验后撤销；其余暂不同步结论见总清单。 |
...
| `33a5d7b585` | 2026-03-24 | Android-only | `fix: KeyboardChatScrollView + RefreshControl conflict (#1398)` | Harmony 基础 Chat 使用 JS spacer，不使用 Android decorator，因此没有 RefreshControl child 查找链路。 | 已分析-不需同步 |
...
| `1d9a9ac21f` | 2026-04-15 | 触达 JS/TS | `feat: freeze in KeyboardChatScrollView as SharedValue (#1434)` | 基础 boolean freeze 已有落点；SharedValue 扩展不需要新增系统能力，后续单独同步验证。 | 已分析-可同步 |
...
| `c436fbb9e0` | 2026-04-20 | 触达 JS/TS+Android | `fix: bug with KeyboardChatScrollView + inverted + maintainVisibleScrollPosition + FlashList (#1437)` | 只作用于 inverted + FlashList；Harmony inverted 因缺少稳定 top inset/负滚动范围而禁用。 | 因系统能力缺失不能完全做 |
| `1048f5238a` | 2026-04-27 | Android-only | `fix: resolve view crash (#1444)` | 已审计：是 `viewPositionInWindow` 的 Android resolveView 异常保护；Harmony 该 API 因普通 tag 测量能力缺失暂不同步，未来实现时必须同步失败降级。 | 已分析-暂不同步 |
| `ce8ccaa5f7` | 2026-04-28 | 触达 JS/TS | `feat: add onContentInsetChange callback (#1445)` | Harmony spacer 有实际生效的 padding 数值，回调已有承接路径，需单独验证打开、关闭和冻结时序。 | 已分析-可实现待验证 |
...
| `5dc56430d7` | 2026-05-04 | 触达 JS/TS | `feat: expose onEndVisible callback from KeyboardChatScrollView (#1450)` | 基础组件已有 scroll/layout/business contentSize，可计算非 inverted 尾部可见状态，需单独验证回调稳定性。 | 已分析-可实现待验证 |
...
- 已分析第 `116-120 / 155` 笔：第 116 `d1fb606b0b` 是上游 plain RN `Animated` 版 `KeyboardStickyView` 的 Fabric enabled=false graph 修复，Harmony 当前 Reanimated 版本不存在该问题。第 117 `11c0dbfba8` 的基础 `KeyboardChatScrollView` 已同步主仓：公共 API 和状态机沿用上游，Harmony 以真实 spacer 扩展 bottom scroll range、扣除业务 contentSize 中的 spacer，并使用 `useScrollViewOffset` 跟踪滚动；真机确认普通非 inverted、默认 always 路线有效。`inverted` 明确为 No。第 118 `a096e393fd` 是 iOS UIKit swizzle，不影响 Harmony。第 119 `83e34b7069` 的 `extraContentPadding` 已有测试落点但不夹入基础提交，后续单独验证。第 120 `8393ee294f` 只修 Android WindowDimensionListener 引用清理，Harmony 当前无直接同步点。
- 已分析第 `121-125 / 155` 笔：第 121 `d4f1a4cf91` 的 helper 初始化顺序修复现在已有真实文件落点，应作为基础组件后的下一笔纯 JS 修复同步。第 122 `7f32767709` 的 `minimumContentPadding` 及后续最终名 `blankSpace` 仍需基于 Harmony spacer 单独验证，不夹入基础提交。第 123 `66e62784c3` 和第 125 `eb2dfbbe35` 的 KAV `automaticOffset/viewPositionInWindow` 仍因普通 RN tag 无法稳定解析而暂不同步；第 124 `f5dca3cdf9` 是纯 TS union 修复，可单独同步但不能夹带 `automaticOffset`。
...
- 已分析第 `136-140 / 155` 笔：第 136 `33a5d7b585` 是 Android decorator 在 RefreshControl 下递归找真实 ScrollView；Harmony Chat 使用 JS spacer，不使用原生 decorator，因此不需同步。第 137 `af5da35fb0` 是 iOS 26 compat 专用；第 138 `ba41d5d1ea` 仍依赖未同步的 KASV contentInset/selection 状态机；第 139 `f83e78c04d` 的 keyboardBorderRadius 可按 Android 返回 0，待测试；第 140 `7b9813396c` 是 iOS responder 模式切换专用。
- 已分析第 `141-145 / 155` 笔：第 141 `75981eeb36` 仅新增上游 KASV resize 测试；第 142 `fbedbc2308` 的 KASV `mode` 默认 insets 仍缺 Harmony 等价底座；第 143 `51fbe47b09` 和第 144 `e979dbf426` 分别是 Android Activity listener 与 edge-to-edge 专用。第 145 `1d9a9ac21f` 的 SharedValue `freeze` 现在已有基础 Chat 组件落点，属于可同步的后续 JS API，但按用户要求单独验证后再提交。
- 已分析第 `146-150 / 155` 笔：第 146 `b3dc83edd5` 的 opening dismiss 已在 Harmony 真机试验后撤销；第 147 `15111a4213` Toolbar 固定 touch bounds 待测试；第 148 `c436fbb9e0` 只修 inverted + FlashList，而 Harmony inverted 明确为 No；第 149 `1048f5238a` 是未同步 viewPositionInWindow 的 Android防崩溃；第 150 `ce8ccaa5f7` 的 `onContentInsetChange` 已有 Harmony spacer 数值承接路径，后续单独验证实际回调时序。
- 已分析第 `151-155 / 155` 笔：第 151 `21a939f30d` 的 Reanimated StickyView 目标已由 Harmony 当前实现覆盖；第 152 `5dc56430d7` 的 `onEndVisible` 可基于基础 Chat 已有 scroll/layout/business contentSize 计算，后续单独验证；第 153、154 分别是 iOS weak keyboard view 与 Catalyst swizzle guard；第 155 是 Android IME onStart 重入修复，均不适用于当前 Harmony 链路。

## 下一步

`1.16.5..1.21.8` 的 `155` 笔源码相关 commit 已全量审计完毕。第 `117 / 155` 基础 `KeyboardChatScrollView` 已完成测试工程真机验证并回写主仓；下一步按时间顺序先同步第 `121 / 155` helper 初始化修复，再逐项验证 `extraContentPadding`、`blankSpace`、SharedValue `freeze`、`onContentInsetChange` 和 `onEndVisible`。`ClippingScrollView` 等价语义及 Chat `inverted` 仍因 RNOH 稳定 top/bottom inset 能力缺失记录为 No。其他候选继续按既有顺序处理。
